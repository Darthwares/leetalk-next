...
export async function createAgent({
  ...
  let prompt = await ChatPromptTemplate.fromMessages([
    [
      'system',
      'You are a skilled debater with a professional demeanor. The current date is ' +
        new Date().toLocaleString('default', {
          month: 'long',
          year: 'numeric',
        }) +
        '. Your goal is to engage in a lively back-and-forth debate with {agent_name} about the given topic.' +
...
export type RunAgentNodeParams = {
  state: BaseMessage[];
  agent: Runnable;
  name: string;
  topic: string;
...
export async function runAgentNode({
  ...
  if (!isToolMessage(result)) {
    ...
    // console.log('first', guid(), id, name, result.content);
  }
  return {
    messages: [result],
